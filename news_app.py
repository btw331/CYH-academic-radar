...
LIGHT_FIELDS = "paperId,title,year,citationCount,venue,authors.name,references.paperId,references.citationCount,references.year,citations.paperId,citations.citationCount,citations.year,url"
RICH_FIELDS = "paperId,title,year,citationCount,venue,authors.name,authors.authorId,abstract,tldr,url"
BROAD_FIELDS = "paperId,title,year,citationCount,venue,authors.name,abstract,tldr,url"
...
st.caption("核心：**同名同姓篩選** + **Streamlit 參數修正** + **時序超連結**。")
...
if st.session_state.read_only_mode:
    ...
    with tab_insight:
        ...
        if st.session_state.get('deep_dive_result'):
            ...
            # ========================================================
            # [核心修改] 替換原有的 st.dataframe，改為 HTML 表格
            # 解決問題：1. 時間顯示異常 (如 2,023)  2. 標題無超連結
            # ========================================================
            st.subheader("📅 關鍵發展時序 (Timeline & Links)")
            ...
            def get_auth_display(p):
                ...
                last_2 = auths[-2].get('name') if len(auths) > 2 else ""
                if last_2:
            ...
            for p in all_papers:
                ...
                if p == st.session_state.full_lineage['hero']:
                    role_text = "🟨 主角"
                    role_style = "background-color: #fff9c4; color: #fbc02d; border: 1px solid #fdd835;"
                elif p in st.session_state.full_lineage['ancestors']:
                    role_text = "🟦 基石"
                    role_style = "background-color: #e3f2fd; color: #1e88e5; border: 1px solid #90caf9;"
                else:
                    role_text = "🟩 後續"
                    role_style = "background-color: #e8f5e9; color: #43a047; border: 1px solid #a5d6a7;"
                
                role_html = f'<span class="{role_class}" style="{role_style}">{role_text}</span>'
                
                # 處理標題與連結
                pid = p.get('paperId', '')
                url = p.get('url') if p.get('url') else f"https://www.semanticscholar.org/paper/{pid}"
                title_html = f'<a href="{url}" target="_blank" title="點擊前往 Semantic Scholar">{p.get("title", "Unknown Title")}</a>'
                
                # 處理年份 (解決 2,023 問題)
                year_val = p.get('year')
                year_display = str(int(year_val)) if year_val else "Unknown"
                
                # 處理作者
                auth_str = get_auth_display(p)
                
                html_rows += f"""
                <tr>
                    <td style="white-space:nowrap; text-align:center;">{year_display}</td>
                    <td style="white-space:nowrap; text-align:center;">{role_html}</td>
                    <td>{title_html}</td>
                    <td style="font-size:0.85em; color:#666;">{auth_str}</td>
                </tr>
                """
            ...
            <div class="scrollable-table-container">
                <table class="custom-table">
                    <thead>
                        <tr>
                            <th style="width:80px; text-align:center;">年份</th>
                            <th style="width:80px; text-align:center;">角色</th>
                            <th>論文標題 (點擊閱讀)</th>
                            <th style="width:25%;">關鍵作者</th>
                        </tr>
                    </thead>
                    <tbody>
                        {html_rows}
